...
<head>
    ...
    <title>Login & Daftar - Berco Cafe</title>
...
<body class="min-h-screen bg-gradient-to-br from-[#FEF3C7] via-[#FEF9E7] to-[#FDE68A] antialiased text-[#422006]">
    ...
    <div class="relative flex flex-col items-center pt-8 md:pt-12 px-4 min-h-screen">
        ...
        {{-- Status Alert --}}
        <div class="w-full max-w-lg bg-gradient-to-r from-[#FEF3C7] to-[#FFFBEC] border-l-4 border-[#F59E0B] rounded-lg p-3.5 mb-6 text-center shadow-xs animate-slideIn">
            <div class="flex items-center justify-center gap-2">
                <i class="fas fa-clock text-[#F59E0B] text-xs"></i>
                <p class="text-[#92400E] text-xs font-medium">
                    Jam Buka Cafe: <span class="font-bold">10.00 - 22.30 WIB</span>
                </p>
            </div>
        </div>

        {{-- Flash Warning / Status Messages --}}
        @if(session('warning'))
            <div class="w-full max-w-lg bg-amber-50 border-l-4 border-amber-500 text-amber-900 p-3 rounded-lg mb-4 text-xs">
        ...
        {{-- Interactive Auth Card (Alpine.js State) --}}
        <div x-data="{ tab: '{{ $errors->has('password_confirmation') || request()->routeIs('register') ? 'register' : 'login' }}' }" 
            ...
            {{-- TAB 1: FORM LOGIN                                        --}}
            {{-- ======================================================== --}}
            <div x-show="tab === 'login'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                ...
                <form action="{{ route('login') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="email" value="{{ old('email') }}"
                            ...
                            placeholder="admin@bercocafe.com atau username kasir/pelanggan" required autocomplete="username">
                        @error('email')
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-1.5 ml-1">
                            <label class="text-xs font-semibold flex items-center gap-1.5 text-zinc-700">
                                <i class="fas fa-lock text-[#78350F] text-xs"></i>
                                Password
                            </label>
                            <a href="{{ route('password.request') }}" class="text-[11px] text-zinc-400 hover:text-orange-700 font-medium transition">
                                Lupa Password?
                            </a>
                        </div>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" id="remember" name="remember" class="w-3.5 h-3.5 text-[#78350F] rounded border-zinc-300" {{ old('remember') ? 'checked' : '' }}>
                        <label for="remember" class="ml-2 text-xs text-zinc-600 cursor-pointer hover:text-zinc-800">
                    ...
                    @if ($errors->any() && !old('name'))
                        <div class="bg-red-50 border border-red-200 text-red-700 p-3 rounded-xl text-xs">
                            <div class="font-bold mb-1 flex items-center gap-1.5">
                                <i class="fas fa-circle-exclamation"></i> Login Gagal
                            </div>
                    ...
                    {{-- Submit Button --}}
                    <button type="submit" class="w-full bg-gradient-to-r from-[#78350F] to-[#5a2308] text-white py-3.5 rounded-xl font-bold text-sm hover:shadow-lg hover:shadow-orange-900/20 transition-all transform hover:scale-[1.01] active:scale-98">
                ...
                <p class="text-center text-[10px] text-zinc-400 mt-2.5 italic">
                    Mode guest memungkinkan memesan menu langsung tanpa akun reward
                </p>
            ...
            {{-- ======================================================== --}}
            <div x-show="tab === 'register'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
