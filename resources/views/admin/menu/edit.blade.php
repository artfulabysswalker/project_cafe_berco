...
@section('page-title', 'Edit Produk Menu')
@section('breadcrumb', 'Menu / Edit Produk')
...
<div class="create-product-page">
    ...
    <div class="form-container-card">
        <div class="form-header">
            <div>
                <h2 class="form-title"><i class="fas fa-pen-to-square text-amber"></i> Edit Produk: {{ $menu->nama_menu }}</h2>
                <p class="form-subtitle">Perbarui informasi nama produk, kategori, harga jual, stok, dan status ketersediaan.</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.menu.update', $menu->id_menu) }}" enctype="multipart/form-data" class="form-content">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid-layout">
                ...
                <div class="form-column">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="nama_menu" name="nama_menu" value="{{ old('nama_menu', $menu->nama_menu) }}" class="form-input" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="id_kategori" name="id_kategori" class="form-select" required>
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ old('id_kategori', $menu->id_kategori) == $cat->id ? 'selected' : '' }}>
                                    {{ $cat->nama_kategori }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    {{-- 3. Pricing: Harga Jual & Info HPP Resep --}}
                    <div class="form-group">
                        ...
                        <div class="input-prefix-wrap">
                            ...
                            <input type="number" id="harga" name="harga" value="{{ old('harga', (int)$menu->harga) }}" class="form-input with-prefix" min="0" required oninput="calculateMargin()" onkeyup="calculateMargin()" onchange="calculateMargin()" onpaste="setTimeout(calculateMargin, 50)">
                        </div>
                    </div>

                    {{-- Info Card HPP Otomatis dari Resep --}}
                    <div class="margin-preview-box" id="marginPreviewBox" style="margin-top: -6px; margin-bottom: 16px;">
                        <div class="margin-item">
                            <span class="margin-lbl flex items-center gap-1.5 text-xs text-stone-500">
                                <i class="fas fa-calculator text-[#B45309]"></i> HPP Resep Terkalkulasi:
                            </span>
                            <strong class="margin-val text-stone-800" id="hppValueText">Rp {{ number_format($menu->hpp ?? 0, 0, ',', '.') }}</strong>
                            <a href="{{ route('admin.hpp') }}" target="_blank" class="text-[11px] text-[#B45309] font-medium hover:underline block mt-0.5">
                                Atur Komposisi Resep &rarr;
                            </a>
                        </div>
                        <div class="margin-item text-right">
                            <span class="margin-lbl">Estimasi Margin Laba</span>
                            <strong class="margin-val text-green" id="profitPctText">{{ $menu->profit_percentage }}% (+Rp {{ number_format($menu->profit_margin, 0, ',', '.') }})</strong>
                        </div>
                    </div>

                    {{-- Visual Warning for Loss (HPP > Harga Jual) --}}
                    <div id="marginWarningAlert" style="display:none; background:#fef2f2; border:1.5px solid #f87171; color:#991b1b; padding:10px 14px; border-radius:10px; font-size:12px; font-weight:700; margin-top:-8px; margin-bottom:16px; align-items:center; gap:8px;">
                        <i class="fas fa-triangle-exclamation text-rose-600 text-sm"></i>
                        <span id="marginWarningText">Peringatan: Harga Jual lebih rendah dari modal HPP resep!</span>
                    </div>
                    ...
                    <div class="stock-status-row">
                        <div class="form-group flex-1">
                            <label class="form-label" for="stok">
                                Stok Saat Ini (Unit/Porsi) <span class="text-danger">*</span>
                            </label>
                            <input type="number" id="stok" name="stok" value="{{ old('stok', $menu->stok ?? 0) }}" class="form-input" min="0" required>
                        </div>

                        <div class="form-group flex-1">
                            <label class="form-label">Status Aktif Produk</label>
                            <label class="toggle-switch-card">
                                <input type="checkbox" name="status_tersedia" value="1" {{ $menu->status_tersedia ? 'checked' : '' }} id="status_tersedia">
                                <span class="slider"></span>
                                <span class="toggle-label" id="statusLabel">{{ $menu->status_tersedia ? 'Tersedia / Aktif' : 'Habis / Nonaktif' }}</span>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-column">
                    ...
                    <div class="form-group">
                        ...
                        <div class="image-upload-dropzone" onclick="document.getElementById('foto').click()">
                            ...
                            <div id="imagePreviewContainer" style="{{ $menu->foto ? 'display:block;' : 'display:none;' }}" class="image-preview-wrap">
                                <img id="imagePreview" src="{{ $menu->foto ? '/storage/' . $menu->foto : '' }}" alt="Preview">
                                <span class="change-photo-btn"><i class="fas fa-camera"></i> Ganti Foto</span>
                            </div>
                            <div id="uploadPlaceholder" style="{{ $menu->foto ? 'display:none;' : 'display:flex;' }}" class="upload-placeholder">
                                <i class="fas fa-cloud-arrow-up upload-icon"></i>
                                <strong>Klik untuk unggah foto baru</strong>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="deskripsi" name="deskripsi" class="form-textarea" rows="4">{{ old('deskripsi', $menu->deskripsi) }}</textarea>
                    </div>
                </div>
            </div>
...
const currentMenuHpp = {{ (float)($menu->hpp ?? 0) }};

function calculateMargin() {
    const harga = parseFloat(document.getElementById('harga').value) || 0;
    const hpp = currentMenuHpp;
    const profitRp = harga - hpp;
    const profitPctEl = document.getElementById('profitPctText');
    const warningAlert = document.getElementById('marginWarningAlert');
    const warningText = document.getElementById('marginWarningText');
    const previewBox = document.getElementById('marginPreviewBox');

    if (!profitPctEl) return;

    if (harga > 0 || hpp > 0) {
        if (hpp > harga) {
            // RUGI / LOSS DETECTED
            const lossRp = Math.abs(profitRp);
            const lossPct = harga > 0 ? Math.round((lossRp / harga) * 100) : 100;

            profitPctEl.innerText = '-' + lossPct + '% (-Rp ' + lossRp.toLocaleString('id-ID') + ')';
            profitPctEl.className = 'margin-val text-red';

            if (warningText && warningAlert) {
                warningText.innerHTML = `<strong>⚠️ PERINGATAN:</strong> Harga Jual (Rp ${harga.toLocaleString('id-ID')}) lebih rendah dari modal HPP (Rp ${hpp.toLocaleString('id-ID')})! Estimasi <strong>RUGI: -Rp ${lossRp.toLocaleString('id-ID')} (-${lossPct}%)</strong>`;
                warningAlert.style.display = 'flex';
            }
            if (previewBox) previewBox.style.borderColor = '#f87171';
        } else {
            // UNTUNG / PROFIT
            const profitPct = harga > 0 ? Math.round((profitRp / harga) * 100) : 0;

            profitPctEl.innerText = profitPct + '% (+Rp ' + profitRp.toLocaleString('id-ID') + ')';
            profitPctEl.className = 'margin-val text-green';

            if (warningAlert) warningAlert.style.display = 'none';
            if (previewBox) previewBox.style.borderColor = 'rgba(212, 165, 116, 0.2)';
        }
    } else {
        profitPctEl.innerText = '0% (+Rp 0)';
        profitPctEl.className = 'margin-val text-green';
        if (warningAlert) warningAlert.style.display = 'none';
        if (previewBox) previewBox.style.borderColor = 'rgba(212, 165, 116, 0.2)';
    }
